...
▶ [자동 트리거] 조회 결과(Graph DB 또는 SQL)에 아래 키워드 중 **하나라도** 존재하면 herb-card 형식(규칙 3)을 **반드시** 사용하세요:
  `product_id=`, `근당가격`, `근당`, `포장단위`, `박스수량`, `포장단가`, `price_per_geun`, `manufacturer`
  → 이 경우 텍스트·글머리 기호·표 형식은 절대 사용 불가.

▶ 위 키워드가 전혀 없는 순수 한의학 지식 답변(효능, 성질, 귀경, 증상, 처방, 제형 등)만 글머리 기호 목록을 사용하세요.
